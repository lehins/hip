...
import Graphics.Image.Vector.Unboxed (VU, fromUnboxedVector, toUnboxedVector)
...
instance Elt RD cs e => Array RD cs e where
  ...
  traverse2 (getDelayed -> arr1) (getDelayed -> arr2) newDims newPx =
    RDImage $ R.traverse2 arr1 arr2 (tSh2 `boobs` (newDims `on` shT2)) newPixel where
