...
import Graphics.Image.Internal (Image, VectorStrategy(..), toUnboxedVector)
...
-- 'IO.setDisplayProgram'. By default 'gpicview' program is used. This is function
...
displayImageHistograms :: (Pixel (Channel px), Pixel px, RealFrac (Channel px),
                           Enum (Channel px)) =>
                          Int -- ^ Number of intervals (bins), that pixels
...
-- | Returns a list of Histograms, one 'Histogram' for each color channel of the
-- image.
...
getHistograms bins img = H.getHistogramsUsing bins img maker where
  ...
  maker = V.toList . H.getHistogramVector (bins+1) delta . toUnboxedVector
