...
instance (
  Readable img px,
  AImage img (Alpha px), OpaquePixel px,
...
jpImageToImage :: (AImage img px, Pixel px, Interconvertible px' px, JP.Pixel px') =>
                  JP.Image px' -> img px
jpImageToImage i = make (fromIntegral $ imageHeight i) (fromIntegral $ imageWidth i) getPx
  where getPx y x = convert $ pixelAt i (fromIntegral x) (fromIntegral y)
...
instance (
  Interconvertible PNM.PPM (img px), AImage img (Alpha px), AImage img px, OpaquePixel px
...
ppmImageToImage (PNM.PPMHeader _ c r) v = make (fromIntegral r) (fromIntegral c) getPx where
  !vectorImage = V.map convert $ VS.convert v
  getPx i j = vectorImage V.! ((fromIntegral i) * r + (fromIntegral j))
...
imageToJPImage f img@(dims -> (m, n)) =
  JP.generateImage g (fromIntegral n) (fromIntegral m) where
    g j i = f $ index img (fromIntegral i) (fromIntegral j)
