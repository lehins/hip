{-# LANGUAGE BangPatterns, DeriveDataTypeable, FlexibleContexts, FlexibleInstances,
             GADTs, StandaloneDeriving, TypeFamilies, MultiParamTypeClasses,
             NoMonomorphismRestriction, UndecidableInstances #-}
...
-- | Every instance of this @RealPixel@ class can be used as a real and
-- imaginary parts of a Complex pixel.
class (Eq (Channel px), Ord (Channel px), Floating (Channel px), Fractional (Channel px),
       Floating px, Fractional px, Pixel px) => RealPixel px where
...
data Complex px where
  (:+) :: RealPixel px => { real :: !px
                          , imag :: !px
                          } -> Complex px 
  --(:*) :: RealPixel px => !px -> !px -> Complex px

--real (r :+ _) = r
--imag (_ :+ i) = i
  
deriving instance Typeable Complex
--deriving instance RealPixel px => Data (Complex px)
...
fromPol :: RealPixel px => px -> px -> Complex px
fromPol !r !theta = (r * cos theta) :+ (r * sin theta)
{-# INLINE fromPol #-}

toRect :: RealPixel px => Complex px -> (px, px)
...
pxOp :: RealPixel px => (px -> px) -> Complex px -> Complex px
pxOp !op (px1 :+ px2) = op px1 :+ op px2
{-# INLINE pxOp #-}

pxOp2 :: RealPixel px => (px -> px -> px) -> Complex px -> Complex px -> Complex px
...
instance RealPixel px => Pixel (Complex px) where
  ...
  fromDouble !v = fromDouble v :+ fromDouble 0
  {-# INLINE fromDouble #-}
  ...
  --  where !arity1 = arity px1
  update (px1 :+ px2) !n !c = if n < arity1
                               then update px1 n c :+ px2
                               else px1 :+ update px2 (n-arity1) c
    where !arity1 = arity px1
  {-# INLINE update #-}
  ...
  fromChannel !c = fromChannel c :+ fromChannel 0
  {-# INLINE fromChannel #-}
...
instance RealPixel px => Floating (Complex px) where
  ...
  exp (x :+ y)  = (expX * cos y) :+ (expX * sin y) where !expX = exp x
  {-# INLINE exp #-}
