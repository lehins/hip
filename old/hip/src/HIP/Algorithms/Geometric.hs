...
resize :: (Interpolation method px, AImage img px, RealFrac (Channel px)) =>
       ...
       -> Ix     -- ^ New image dimensions @m@ rows
       -> Ix     -- ^ and @n@ columns.
...
downsampleF :: AImage img px => Ix -> Ix -> img px -> img px
{-# INLINE downsampleF #-}
...
upsampleF :: AImage img px => Ix -> Ix -> img px -> img px
{-# INLINE upsampleF #-}
...
pad :: AImage img px =>
    ...
    -> Ix     -- ^ Image's new dimensions @m@ rows
    -> Ix     -- ^ and @n@ columns
...
flipUsing :: AImage img px =>
             (Ix -> Ix -> Ix -> Ix -> (Ix, Ix)) -> img px -> img px
