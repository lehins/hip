...
import HIP.Interface (Ix)
import HIP.Pixel.Base (Pixel(..))


class Pixel px => Interpolation method px | px -> method where
  interpolate :: (Num Ix, Integral Ix, Pixel px) =>
                 method             -- ^ Interpolation method
              -> Ix -> Ix         -- ^ Image dimensions @m@ rows and @n@ columns.
              -> (Ix -> Ix -> px) -- ^ Lookup function that returns a pixel at @i@th
                                    -- and @j@th location.
