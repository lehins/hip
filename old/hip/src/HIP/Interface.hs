...
class (Num (img px), Show (img px), Pixel px,
       Num Ix, Integral Ix, Ord Ix, Enum Ix, Bits Ix, Bounded Ix, Show Ix) =>
      AImage img px | px -> img where
  type Ix :: *
  ...
  imap :: (AImage img px1) => (Ix -> Ix -> px1 -> px) -> img px1 -> img px
  ...
  backpermute :: Ix -> Ix -- ^ rows and columns in a new image.
              -> (Ix -> Ix -> (Ix, Ix)) -- ^ Function that maps each
  ...
  -- source image.
  crop :: Ix     -- ^ @i@ and 
       -> Ix     -- ^ @j@ starting index from within an old image.
       -> Ix     -- ^ @m@ rows and
       -> Ix     -- ^ @n@ columns - dimensions of a new image.
       -> img px  -- ^ Source image.
       -> img px
  ...
  fromBoxedVector :: Ix -> Ix -> V.Vector px -> img px
