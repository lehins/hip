...
data AccImage px where
  AccArrayImage :: Pixel px => !(Acc (Array DIM2 px)) -> AccImage (Exp px)
  AccSingleton  :: Pixel px => !(Acc (Scalar px))     -> AccImage (Exp px)
...
instance (Pixel px) => AImage AccImage (Exp px) where
  type Ix = Exp Int
